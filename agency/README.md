...
| 2c. Cloud sync | `npm run app:deploy` + `npm run push` | Optional: host the dashboard so it's **always up to date on every device** (phone + laptop stay in sync). Data lives in a private Vercel KV store behind your login; the pipeline pushes leads up with `npm run push`. One-time setup in [SYNC-SETUP.md](SYNC-SETUP.md). A badge shows Synced vs Local-only so there's never any doubt |
